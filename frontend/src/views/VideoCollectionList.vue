  ...
  <div class="collection-list">
    <el-card>
      ...
      <el-table :data="tableData" border stripe v-loading="loading">
        ...
        <el-table-column prop="video_count" label="影片数" width="100" align="center" />
...
import { getCollectionList, deleteCollection, updateCollectionStatus } from '../api'
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await getCollectionList(queryForm)
...
const handleEdit = (row) => {
  router.push(`/collections/${row.id}/edit`)
}
...
const handleToggleStatus = async (row) => {
  ...
  try {
    await ElMessageBox.confirm(`确定要${action}该合集吗？`, '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
    await updateCollectionStatus(row.id, newStatus)
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm('确定要删除该合集吗？删除后将无法恢复！', '警告', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'error'
    })
...
.collection-list :deep(.el-table) {
  border-radius: 8px;
}

.collection-list :deep(.el-table th.el-table__cell) {
  background: #f8fafc;
  color: #475569;
  font-weight: 600;
  font-size: 13px;
...
.collection-list :deep(.el-button--primary) {
  background: #6366f1;
  border-color: #6366f1;
}

.collection-list :deep(.el-button--primary:hover) {
  background: #4f46e5;
  border-color: #4f46e5;
}
</style>
